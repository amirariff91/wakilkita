...
const safeguards = [
  "One verified constituent account per IC-backed identity",
  "Constituency eligibility checked before support is counted",
  "Identity records separated from support records",
  "Public results show aggregate mandate signal only",
  "Every count-changing action produces an audit event",
    ...
    <main className="min-h-screen overflow-hidden">
      <section className="mx-auto flex w-full max-w-7xl flex-col gap-14 px-5 py-6 sm:px-8 lg:px-10">
        <nav className="flex items-center justify-between rounded-full border border-[var(--line)] bg-[rgba(255,250,241,0.76)] px-4 py-3 shadow-[0_18px_80px_rgba(7,22,19,0.08)] backdrop-blur">
          ...
          <a
            href="mailto:hello@wakilkita.com?subject=Join%20WakilKita%20pilot"
        ...
        <div id="top" className="grid items-center gap-10 py-8 lg:grid-cols-[1.04fr_0.96fr] lg:py-14">
          <div>
            <div className="mb-6 inline-flex rounded-full border border-[rgba(15,107,77,0.24)] bg-[rgba(221,247,232,0.72)] px-4 py-2 text-sm font-bold text-[var(--civic-dark)]">
              Not online voting. Not SPR. A verified public preference signal.
            </div>
            <h1 className="max-w-4xl font-serif text-5xl font-black leading-[0.95] tracking-[-0.07em] text-[var(--ink)] sm:text-6xl lg:text-7xl">
              Let constituents show who they actually want to represent them.
            </h1>
            <p className="mt-7 max-w-2xl text-lg leading-8 text-[var(--slate)] sm:text-xl">
              WakilKita helps Malaysians nominate and support one preferred local rep in their own constituency, backed by identity checks, privacy-preserving tallying, and constituency-level issue data.
            </p>
          ...
          <div className="relative rounded-[2rem] border border-[var(--line)] bg-[rgba(255,250,241,0.88)] p-4 shadow-[0_30px_100px_rgba(7,22,19,0.16)]">
            <div className="rounded-[1.5rem] bg-[var(--ink)] p-5 text-white">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--mint)]">Pilot constituency</p>
              ...
              <div className="mt-5 rounded-3xl bg-[var(--paper)] p-5 text-[var(--ink)]">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--civic)]">Leading public mandate signal</p>
                    <h3 className="mt-2 text-3xl font-black tracking-[-0.06em]">Aisyah Rahman</h3>
                    <p className="mt-2 text-sm leading-6 text-[var(--slate)]">Community organiser focused on transit reliability, flood mitigation, and transparent local service KPIs.</p>
                  </div>
                ...
                <p className="mt-3 text-xs font-semibold text-[var(--slate)]">Aggregate only. Individual support choices are never public.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-5 sm:px-8 lg:grid-cols-[0.8fr_1.2fr] lg:px-10">
          <div>
            ...
            <p className="mt-5 text-lg leading-8 text-[var(--slate)]">
              WakilKita should win because the count is hard to abuse and easy to explain. No blockchain theatre. No public preference exposure. Just boring, auditable civic infrastructure.
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {safeguards.map((item, index) => (
              <div key={item} className="rounded-3xl border border-[var(--line)] bg-[var(--paper)] p-5 shadow-[0_16px_50px_rgba(7,22,19,0.07)]">
                <span className="text-xs font-black uppercase tracking-[0.18em] text-[var(--amber)]">0{index + 1}</span>
      ...
      <section id="constituency" className="mx-auto grid max-w-7xl gap-8 px-5 py-16 sm:px-8 lg:grid-cols-[1fr_1fr] lg:px-10">
        <div className="rounded-[2rem] border border-[var(--line)] bg-[var(--paper)] p-6 shadow-[0_24px_80px_rgba(7,22,19,0.1)]">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-black tracking-[-0.05em] text-[var(--ink)]">Local mandate snapshots</h2>
            </div>
            <span className="rounded-full bg-[var(--mint)] px-3 py-2 text-xs font-black text-[var(--civic-dark)]">demo data</span>
          </div>
            ...
            {constituencies.map((seat) => (
              <div key={seat.code} className="rounded-3xl border border-[var(--line)] bg-white/60 p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--amber)]">{seat.code}</p>
        ...
        <div className="rounded-[2rem] bg-[var(--ink)] p-6 text-white shadow-[0_24px_90px_rgba(7,22,19,0.2)]">
          ...
          <h2 className="mt-3 text-3xl font-black tracking-[-0.05em]">Issues ranked by data and resident priority</h2>
        ...
        <div className="rounded-[2.4rem] border border-[var(--line)] bg-[var(--paper)] p-6 shadow-[0_30px_100px_rgba(7,22,19,0.12)] sm:p-8 lg:p-10">
          <div className="grid gap-8 lg:grid-cols-[0.82fr_1.18fr]">
            <div>
              ...
              <p className="mt-5 text-lg leading-8 text-[var(--slate)]">
                First milestone: one constituency, real resident verification, real nominations, and a public aggregate mandate signal that can survive scrutiny.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              {roadmap.map((item) => (
          ...
          <div className="mt-8 rounded-3xl bg-[var(--ink)] p-5 text-white sm:flex sm:items-center sm:justify-between sm:gap-6">
            <div>
              ...
              <p className="mt-2 text-xl font-black tracking-[-0.03em]">One eligible resident supports one local preferred rep. It is a mandate signal, not an official vote.</p>
            </div>
            <a
              href="mailto:hello@wakilkita.com?subject=Pilot%20WakilKita"
